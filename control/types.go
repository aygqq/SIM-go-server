...
const (
	...
	CMD_CFG_ERROR      = 12 // [cmd, len, 0]
	CMD_CTRL_ERROR     = 13 // [cmd, len, 0]
	CMD_PC_WAITMODE    = 14 // [cmd, len, 1]
	CMD_PC_SHUTDOWN    = 15 // [cmd, len, 1]
	CMD_PC_READY       = 16 // [cmd, len, 1]
	CMD_NEW_PHONES     = 17 // [cmd, len, data]
	CMD_SEND_SMS       = 18 // [cmd, len, idx, type, phone, msg]
	CMD_REQ_MODEM_INFO = 19 // [cmd, len, idx]
	CMD_REQ_PHONES     = 20 // [cmd, len, 0]
	CMD_REQ_REASON     = 21 // [cmd, len, 0]
	CMD_OUT_SHUTDOWN   = 22 // [cmd, len, 1]
	CMD_OUT_SAVE_STATE = 23 // [cmd, len, data]
	CMD_OUT_SIM_CHANGE = 24 // [cmd, len, data]
	CMD_OUT_SMS        = 25 // [cmd, len, idx, type, phone, msg]
	CMD_OUT_AT_CMD     = 26 // [cmd, len, ]
